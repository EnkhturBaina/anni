...
function App() {
  ...
  function MyStopwatch() {
    ...
    const displayTime = date1.getHours() - date2.getHours();
    const displayMinute = date1.getMinutes() - date2.getMinutes();
    const displaySec = date2.getSeconds();
    if (displayTime < 0) {
      displayTime1 = displayTime * -1;
    } else {
      displayTime1 = displayTime;
    }
    if (displayMinute < 0) {
      displayMinute1 = displayMinute * -1;
    } else {
      displayMinute1 = displayMinute;
    }
    return (
      <div style={{ textAlign: "center", marginTop: 50 }}>
        <div className="headerText-container" style={{ fontSize: "14px" }}>
          <span>{diffDays} Өдөр</span>
          <span> {displayTime1} цаг</span>
          <span> {displayMinute1} минут</span>
          <span> {displaySec} секунд</span>
        </div>
      </div>
    );
  }
